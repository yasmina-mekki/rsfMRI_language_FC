...
def gcta_bivariate_analysis(grm, pheno, covar, qcovar, output, constraint=None):
    """
    Parameters
    ...
    if constraint is None:
        ...
        cmd = " ".join(['/path/bin/gcta64',
                        '--grm-gz %s' % grm,
                        '--pheno %s' % pheno,
                        '--covar %s' % covar,
                        '--qcovar %s' % qcovar,
                        '--reml-bivar-lrt-rg %s' %constraint,
                        '--out %s' %output,    
                         '--thread-num 10'])
...
if __name__ == '__main__':
    ...
    gcta_bivariate_analysis(kinship_m_path,
                            phenotype_path,
                            covars_tmp_path,
                            qcovars_tmp_path,
